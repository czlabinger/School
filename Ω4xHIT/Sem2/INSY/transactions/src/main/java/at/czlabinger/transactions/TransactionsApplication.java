package at.czlabinger.transactions;
...
public class TransactionsApplication {
	...
	Connection setupDB() {
		...
		try {
			dbProps.load(new FileInputStream(rootPath + "../../src/main/resources/db.properties"));
	...
	public static void main(String[] args) throws Throwable {
		TransactionsApplication webshop = new TransactionsApplication();
		webshop.start();
		...
		public void handle(HttpExchange t) throws IOException {
			...
			try {
				...
				ResultSet rs = st.executeQuery(
						"SELECT id, description, price, amount FROM articles");
				while (rs.next()) {
	...
	class OrdersHandler implements HttpHandler {
		...
		public void handle(HttpExchange t) throws IOException {
			...
			try {
				Statement st = conn.createStatement();
				String query =
						"SELECT o.id, c.name, count(*), sum(l.amount * a.price) "
	...
	class PlaceOrderHandler implements HttpHandler {
		@Override
		public void handle(HttpExchange t) throws IOException {
			System.out.printf("Placing order using thread %s / %d%n",
			...
			try {
				...
				// Stellt sicher, dass nur eine Bestellubng/Transaktion Zugang zu den Artikeln hat
				conn.createStatement().executeUpdate("LOCK TABLE articles IN ROW EXCLUSIVE MODE");


				Statement st1 = conn.createStatement();

				// Get the next free order id
				ResultSet rs1 = conn.createStatement().executeQuery("SELECT MAX(id) from orders");
				if (rs1.next())
				...
				for (int i = 1; i <= (params.size() - 1) / 2; ++i) {
					...
					PreparedStatement stGetAmount =
							conn.prepareStatement("SELECT amount FROM articles WHERE id = ?");
					...
					// Simulate some slow task here like warehouse lookup for available
					// items.
					sleep(5);

					if (available < amount)
						throw new IllegalArgumentException(String.format(
								"Not enough items of article #%d available", article_id));

					PreparedStatement stSetAmount = conn.prepareStatement(
							"UPDATE articles SET amount = amount - ? WHERE id = ?");
					stSetAmount.setInt(1, amount);conn.commit();
					...
					PreparedStatement stInsert = conn.prepareStatement(
							"INSERT INTO order_lines (order_id,article_id,amount) VALUES (?,?,?)");
				...
				try {
					// Freigabe der Zeilensperre auf die Tabelle articles
					conn.createStatement().executeUpdate("UNLOCK TABLE articles");
			...
			System.out.printf("Finished placing order using thread %s / %d%n",
					Thread.currentThread().getName(),
					Thread.currentThread().getId());
		}
	}
		...
		public void handle(HttpExchange t) throws IOException {
			String response =
					"<!doctype html>\n"
							...
							+
							"<dt>Bestellstatistiken anzeigen:</dt><dd><a href=\"http://127.0.0.1:" +
							port + "/stats\">http://127.0.0.1:" + port + "/stats</a></dd>"
							+ "<dt>Alle Artikel anzeigen:</dt><dd><a href=\"http://127.0.0.1:" +
							port + "/articles\">http://127.0.0.1:" + port + "/articles</a></dd>"
							+
							"<dt>Alle Bestellungen anzeigen:</dt><dd><a href=\"http://127.0.0.1:" +
							port + "/orders\">http://127.0.0.1:" + port + "/orders</a></dd>"
							+
							"<dt>Details zu Bestellung anzeigen:</dt><dd><a href=\"http://127.0.0.1:" +
							port + "/order?order_id=<order_id>\">http://127.0.0.1:" + port +
							"/order?order_id=&lt;order_id></a></dd>"
							+ "<dt>Alle Kunden anzeigen:</dt><dd><a href=\"http://127.0.0.1:" +
							port + "/clients\">http://127.0.0.1:" + port + "/clients</a></dd>"
							+ "<dt>Bestellung abschicken:</dt><dd><a href=\"http://127.0.0.1:" +
							port +
							"/placeOrder?client_id=<client_id>&article_id_1=<article_id_1>&amount_1=<amount_1&article_id_2=<article_id_2>&amount_2=<amount_2>\">http://127.0.0.1:" +
							port +
							"/placeOrder?client_id=&lt;client_id>&article_id_1=&lt;article_id_1>&amount_1=&lt;amount_1>&article_id_2=&lt;article_id_2>&amount_2=&lt;amount_2></a></dd>"
							+ "</dl></body></html>";
	...
	class StatsHandler implements HttpHandler {
		...
		public void handle(HttpExchange t) throws IOException {
			...
			Statement st = null;
			try {
				// Es geht nur darum dass die Bestellungen consistent sind und komplett commited
				...
				ResultSet rs = st.executeQuery(
						("SELECT c.country, count(distinct c.id), count(distinct o.id) "
				...
				while (rs.next()) {
					...
					PreparedStatement stDetail = conn.prepareStatement(
							"SELECT c.name, o.id, count(*), sum(l.amount * a.price) "
	 ...
	 * @throws IOException
	 */
	private void answerRequest(HttpExchange t, String response)
			throws IOException {
